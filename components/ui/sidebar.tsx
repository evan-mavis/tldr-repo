...
function SidebarMenuAction({
  ...
  onKeyDown,
  ...props
}: Omit<React.ComponentProps<"button">, "onClick" | "onKeyDown"> & {
  asChild?: boolean;
  showOnHover?: boolean;
  onClick?: (e: React.MouseEvent<HTMLElement>) => void;
  onKeyDown?: (e: React.KeyboardEvent<HTMLElement>) => void;
}) {
  // When not using asChild, render as a span to avoid nested buttons
  // The parent SidebarMenuButton is already a button, so we use a span
  ...
  return (
    <Comp
      ...
      className={cn(
        "text-sidebar-foreground ring-sidebar-ring hover:bg-sidebar-accent hover:text-sidebar-accent-foreground peer-hover/menu-button:text-sidebar-accent-foreground absolute top-1.5 right-1 aspect-square w-5 cursor-pointer items-center justify-center rounded-none p-0 outline-hidden transition-transform group-data-[collapsible=icon]:hidden peer-data-[size=default]/menu-button:top-1.5 peer-data-[size=lg]/menu-button:top-2.5 peer-data-[size=sm]/menu-button:top-1 after:absolute after:-inset-2 focus-visible:ring-2 md:after:hidden [&>svg]:size-4 [&>svg]:shrink-0",
        showOnHover &&
